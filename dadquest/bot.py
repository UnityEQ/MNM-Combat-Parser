"""
DadQuest — Chat Parser for Monsters & Memories.
...
# ===================================================================
# Debug logger — writes to dadquest/logs/
# ===================================================================
def _setup_bot_log():
    ...
    logger = logging.getLogger("dadquest")
...
class BotApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(title_frame, text="\u2500\u2500 DadQuest \u2500\u2500", bg=COLORS["bg"],
...
def _check_for_update():
    ...
    try:
        ...
        old_path = os.path.join(exe_dir, "DadQuest.exe.old")
        new_path = os.path.join(exe_dir, "DadQuest.exe.new")
        ...
        url = "https://zekparser.com/DadQuest.exe"
        req = urllib.request.Request(url, headers={
            "User-Agent": "DadQuest AutoUpdater"})
        ...
        os.rename(new_path, os.path.join(exe_dir, "DadQuest.exe"))

        # Restart with new exe
        subprocess.Popen([os.path.join(exe_dir, "DadQuest.exe")])
        sys.exit(0)
    ...
    if not _is_admin():
        if getattr(sys, 'frozen', False):
            ctypes.windll.user32.MessageBoxW(
                0, "DadQuest requires Administrator privileges.\n"
